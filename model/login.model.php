...
public function AreaProduccion($usuario,$areap)
{
	try
	{
		$stm = $this->conexionn->prepare("SELECT usuario FROM v_usuarios WHERE usuario = ? AND id_areap = ?");
		$stm->execute(array($usuario,$areap));
		if ($stm){		
			$hay_datos=0;
			while ($row = $stm->fetchAll(PDO::FETCH_OBJ)){
				$hay_datos=1;
