...
$(function() {
    ...
    $('.DatePicker').on('changeDate', function(e) {
        $('#frm-compra').formValidation('revalidateField', 'compra_fecha');
    });

    $('.clockpicker').on('change', function(e) {
        $('#frm-compra').formValidation('revalidateField', 'compra_hora');
    });

    $('#frm-compra').formValidation({
        framework: 'bootstrap',
        fields: {
        }
    })
        ...
        else if($('#cod_prov').val() == 0)
        {
            ...
            if($('input:radio[name=tipo_compra]:checked').val() == 2){
                ...
                $('#mensaje-cre').html('<div class="alert alert-warning">'
                    +'<i class="fa fa-warning"></i> Ingresar el monto total de la compra. <a class="alert-link" data-toggle="modal" data-target="#mdl-credito">AQUI</a>.'
                    +'</div>');
                return false;
            }else{
...
$('#credito').on('ifChecked', function(event){
    ...
    $('#frm-compra').formValidation('revalidateField', 'monto_cre');
});
...
$('#contado').on('ifChecked', function(event){
    ...
    $('#monto_con').val('');
    $('#desc_comp').val('0.00');
    $('#frm-compra').formValidation('revalidateField', 'monto_con');
...
$("#busc_prov").autocomplete({
    ...
    source: function (request, response) {
        $.ajax({     
            type: "post",
            dataType: "json",
            url: '?c=Compra&a=buscarProveedor',
            data: {
                cadena: request.term
            },
            success: function (data) {
...
$('#mdl-credito').on('hidden.bs.modal', function() {
    ...
    if($('#monto_cre').val() == '0.00' || $('#monto_cre').val() == ''){
        ...
        $('#mensaje-cre').html('<div class="alert alert-info">'
            +'<i class="fa fa-info"></i> Puedes modificar las cuotas de la compra. <a class="alert-link" data-toggle="modal" data-target="#mdl-credito">AQUI</a>.'
            +'</div>');
    }
});
...
var montoTotal = function(){
    ...
    $(".igv_tc").text(moneda+' '+formatNumber(igv));
    $(".sb_tc").text(moneda+' '+formatNumber(sbt));   
}
