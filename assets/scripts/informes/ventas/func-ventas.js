...
var listar = function(){
    ...
    $.ajax({
        ...
        success: function(item){
            ...
            $('#des_v').text(moneda+' '+formatNumber(v_des));
            $('#total_v').text(moneda+' '+formatNumber(vtotal));
            $('#subt_v').text(moneda+' '+formatNumber(vsbt));
            $('#igv_v').text(moneda+' '+formatNumber(vigv));
        }
    });
    ...
    .DataTable({
        ...
                return '<p class="mayus">'+data+'</p>';
            ...
            },
            {
                ...
                "render": function ( data, type, row) {
                    ...
                    switch(opc){
                        case '1':
                            return '<p class="text-right"><strong>Efectivo('+moneda+'):</strong> '+formatNumber(data.pago_efe)+'</p>';
                        break;
                        case '2':
                            return '<p class="text-right"><strong>Tarjeta('+moneda+'):</strong> '+formatNumber(data.pago_tar)+'</p>';
                        break;
                        case '3':
                            return '<p class="text-right"><strong>Efectivo('+moneda+'):</strong> '+formatNumber(data.pago_efe)+'<br>'
                            +'<strong>Tarjeta('+moneda+'):</strong> '+formatNumber(data.pago_tar)+'</p>';
                        break;
                        case '4':
                            return '<p class="text-right"><strong>Vale('+moneda+'):</strong> '+formatNumber(data.pago_efe)+'</p>';
                        break;
                    }
                }
            },
            {"data":null,"render": function ( data, type, row) {
                switch(true){
                    case data.descu > 0:
                        return '<div class="text-right bold"> '+moneda+' '+formatNumber(data.total)+'</div><p class="text-right">Dscto.: -'+formatNumber(data.descu)+'</p>';
                    break;
                    default:
                        return '<p class="text-right bold"> '+moneda+' '+formatNumber(data.total)+'</p>';
                    break;
                }
            }},
            {"data":null,"render": function ( data, type, row) {
        ...
                return '<p class="text-center"><span class="label label-primary">ACTIVO</span></p>';
            ...
            {"data":null,"render": function ( data, type, row ) {
                return '<p class="text-right"><a class="btn btn-sm btn-primary" onclick="detalle('+data.id_ven+',\''+data.desc_td+'\',\''+data.numero+'\')">Ver</a>'
                +'&nbsp;<a href="?c=Informe&a=Imprimir&Cod='+data.id_ven+'" class="btn btn-sm btn-success" target="_blank"><i class="fa fa-print"></i></a></p>';
            }}
        ]
    });
};
