...
$('#tipo_doc, #estado').change( function() {
	listar();
});
...
var listar = function(){
	...
	.DataTable({
        ...
                return '<p class="text-right bold">'+moneda+' '+formatNumber(data)+'</p>';
            }},
            {
                "data": null,
                "render": function ( data, type, row) {
            ...
            },
            {
                ...
                "render": function ( data, type, row ) {
                    if(data.estado == 'a'){
                        return '<p class="text-center"><span class="label label-primary">APROBADA</span></p>';
            ...
                    return '<p class="text-center">'
                    +'<a class="btn btn-xs btn-success" onclick="detalle_c('+data.id_compra+')"><i class="fa fa-files-o"></i></a>'
                    +'&nbsp<a class="btn btn-xs btn-info" onclick="detalle('+data.id_compra+')"><i class="fa fa-eye"></i> Ver</a>'
                    +'</p>';
                }
            }
		]
                ...
                .append(
                  $('<tr/>')
                    ...
                    .append($('<td class="text-right"/>')
                        .html('<p class="text-right"><a class="btn btn-sm btn-info" onclick="detalle_sc('+item.id_credito+')"><i class="fa fa-eye"></i></a></p>'))
                    );
            });
        }
    });
...
/* Detalle sub cuotas */
var detalle_sc = function(cod){
    var moneda = $("#moneda").val();
    $('#list_cdc').empty();
    $('#detalle_sc').modal('show');
        $.ajax({
        type: "POST",
        dataType: "json",
